...
def do_stuff(asset, timeframe, strike):
    ...
    for strikes in range(length):
        ...
        if strike == strike_price:
            ...
            SECTOKEN = L.cookies['X-SECURITY-TOKEN']
            USERNAME = 'SPARROWSWORD'
            DIRECTION = '+'
            HOW_MANY_CONTRACTS = '1'
        ...
        response = L.post('https://trade.nadex.com/iDeal/orders/workingorders/dma', headers=headers, data=data)

        print(response.text)


if __name__ == "__main__":
    do_stuff(asset, timeframe, strike)
